...
void CActor::KeyDrag( vector3d *pDelta )
{
	CKey *pScan ;
...
void CActor::CalcSpline( vector3d *p1, vector3d *p2, vector3d *v1, vector3d *v2, int num, vector3d *points)
{
...
void CActor::CalculateSplineCache( )
{
	...
	if( Pos )
	{
		...
		if( Pos )
		{
			...
			if( m_pCache ) free( m_pCache ) ;
			m_pCache = (vector3d*)malloc( (m_nCache) * sizeof(vector3d) ) ;
			if( m_pCache )
			{
...
void CActor::DrawPath( CRenderContext *pRC, bool Selected )
{
	...
	if( Selected )
	{
		...
		while( Pos )
		{
			...
			//---	Draw the Vector
			pRC->RENDER_Arrow( V3_Length( &v ), RGB(128,128,128) ) ;

			//---	Remove vector rotation Matrix
			pRC->PopMatrix( ) ;
...
void CActor::GetRotationAtTime( vector3d *pRot, int t )
{
	V3_Zero( pRot ) ;
...
void CActor::GetMotionInfoAtTime( CMotion **ppMotion, int *pnFrame, vector3d *pDeltaMove, radian *pFaceDir, radian *pMoveDir, int t )
{
	if( m_pCharacter && m_pCharacter->GetSkeleton() )
	{
		CKey *pActiveKey = GetMotionKeyAtTime( t ) ;
...
void CActor::GetPositionAtTime( vector3d *pv, int t )
{
	//---	Ensure Spline Cache is valid
	if( m_pCache == NULL )
	{
...
void CActor::GetDirectionAtTime( vector3d *pv, int t )
{
	vector3d	v1, v2 ;
