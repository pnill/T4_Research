...
void CAnimBar::SetMotion( CMotion *pMotion )
{
	...
	if( pMotion )
	{
		m_NumFrames = pMotion->GetNumFrames() ;
	}
...
void CAnimBar::OnHScroll(UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	if( m_pView )
	{
		if( m_pView->m_pCharacter )
			m_pView->m_pCharacter->SetCurFrame( (f32)nFrame ) ;
		m_pView->Invalidate() ;
	}
...
void CAnimBar::OnChangeFrame( )
{
	...
	if( m_pView )
	{
		if( m_pView->m_pCharacter )
		{
			CString String ;
			pFrame->GetWindowText( String ) ;
			if( atoi( String ) != m_pView->m_pCharacter->GetCurFrame( ) )
			{
				m_pView->m_pCharacter->SetCurFrame( (f32)atoi(String) ) ;
				m_pView->Invalidate() ;
			}
		}
	}
