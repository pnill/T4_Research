...
BEGIN_MESSAGE_MAP(CAttachJointDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_ATTACH_TO_JOINT, OnButtonAttachToJoint)
...
void CAttachJointDlg::InitBoneCombo()
{
	...
	{
		if( !m_pView->m_pCharacter->m_pCurMotion->m_pLinkCharacter )
			return;

		pSkeleton = m_pView->m_pCharacter->m_pCurMotion->m_pLinkCharacter->m_pSkeleton;
	}
	else
	...
	CString IndexNewName;
	pBoneListCombo->GetLBText( Index, IndexNewName );

	if( !pSkeleton )
		pBoneListCombo->SetCurSel( -1 );
	else if(( Index != -1 ) && ( Index < pBoneListCombo->GetCount()) && ( IndexName == IndexNewName ))
		pBoneListCombo->SetCurSel( Index );
...
void CAttachJointDlg::OnButtonAttachToJoint() 
{
	...
	if( LinkedChar )
	{
		CEvent* pNewEvent = m_pView->m_pCharacter->m_pCurMotion->m_pLinkMotion->AddEvent();
		*pNewEvent <= *m_pSelectedEvent;

		//---	new functionality, move the event to the selected bone
		pNewEvent->SetAttached( TRUE );
		pNewEvent->SetBone( iBone );
		pNewEvent->UpdateEventPosition();
		pNewEvent->SetBone( -1 );
		pNewEvent->SetAttached( FALSE );

		*m_pSelectedEvent <= *pNewEvent;
		m_pView->m_pCharacter->m_pCurMotion->m_pLinkMotion->DeleteEvent( pNewEvent );
	}
	else
	{
		//---	new functionality, move the event to the selected bone
