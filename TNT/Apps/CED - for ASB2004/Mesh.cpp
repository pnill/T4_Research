...
{
	int Loc = m_PathName.Find( rSearch );
...
s32 CMesh::AddVertex( vector3d *pVertex )
{
	//---	Scan for Match
...
void CMesh::AddLine( vector3d *pV1, vector3d *pV2 )
{
	//---	Add Vertices
...
void CMesh::Render( CRenderContext *pRC, COLORREF Color )
{
	...
	if( EnsureLoaded( ) )
	{
		...
		if( (m_pVerts == NULL) && (pR->NQTris != 0) )
		{
			...
			m_pVerts = new vector3d[pR->NQTris * 3] ;
			m_ptVerts = new vector3d[pR->NQTris * 3] ;
		...
		CPen *pOldPen = pRC->m_pDC->SelectObject( &Pen ) ;
