...
void CMovieView::SetDragPlane( vector3d *pVector )
{
}

bool CMovieView::GetCursorOnDragPlane( vector3d *pCursor, CPoint p, f32 y )
{
...
BEGIN_MESSAGE_MAP(CMovieView, CCeDView)
	...
	ON_COMMAND(ID_MOVIE_EXPORTPC, OnMovieExportXBOX)
...
void CMovieView::DrawView( CRenderContext *pRC )
{
	...
	if( m_pMovie->m_UseShotList )
	{
		...
		m_Camera.m_FOV = m_pMovie->ShotGetFOV( GetFrame() );
		m_Camera.m_DX = (r.Width()/2) / x_tan(DEG_TO_RAD(m_Camera.m_FOV)/2.0f) ;
		m_Camera.m_DY = (r.Width()/2) / x_tan(DEG_TO_RAD(m_Camera.m_FOV)/2.0f) ;
        m_Camera.m_OX = (f32)r.left ;
        m_Camera.m_OY = (f32)r.top ;
		...
		if( pEye && pTarget )
		{
			...
			m_Camera.m_Target = vt ;
			m_Camera.m_Distance = V3_Length( &v ) ;
			m_Camera.m_Rotation = -RY ;
			m_Camera.m_Elevation = -RX ;
			m_Camera.ComputeMatrix( ) ;
		}
	}
	else
	{
		m_Camera.m_FOV = 60;			// Default field of view
	}
	...
	//---	Draw Ground Plane
//	M4_SetRotateXYZ( &m, 0, 0, 0 ) ;
//	M4_SetTranslation( &m, 0, 0, 0 ) ;
	pRC->PushMatrix( &m ) ;
	{
		CPen PenMajor( PS_SOLID, 1, RGB( 128, 255, 128 ) ) ;
		CPen PenMinor( PS_SOLID, 1, RGB( 224, 255, 224 ) ) ;
		CPen *pOldPen = pDC->SelectObject( &PenMinor ) ;
		pRC->RENDER_GridLines( -960, 960,  48, -2160, 2160,  48 ) ;
		pDC->SelectObject( &PenMajor ) ;
		pRC->RENDER_GridLines( -960, 960, 240, -2160, 2160, 240 ) ;
...
*/
				...
				M4_SetRotationsXYZ( &m, (radian3d*)(&r) ) ;
				if ( pActor->GetType() == ACTOR_SCENERY )
				{
                    vector3d s;
...
//				M4_SetRotateXYZ( &m, 0, Yaw, 0 ) ;
    ...
    if( m_pMovie->m_UseShotList )
    {
//        pDC->Draw3dRect( &r, RGB( 0, 0, 0 ), RGB( 0, 0, 0 ) ) ;
        pDC->FillSolidRect( 0, 0, r.left, cr.Height(), RGB(0,0,0) ) ;
        pDC->FillSolidRect( r.right, 0, cr.Width()-r.right, cr.Height(), RGB(0,0,0) ) ;
        pDC->FillSolidRect( r.left, 0, r.Width(), r.top, RGB(0,0,0) ) ;
        pDC->FillSolidRect( r.left, r.bottom, r.Width(), cr.Height()-r.bottom, RGB(0,0,0) ) ;
    }
}
...
void CMovieView::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	if( (nFlags & (MK_LBUTTON|MK_RBUTTON)) && (m_ViewState == MOVS_DRAG_KEY) && (GetCapture() == this) )
	{
		...
		if( GetCursorOnDragPlane( &Cursor, point, v.Y ) )
		{
			...
			V3_Sub( &Cursor, &Cursor, &m_DragOffset ) ;
			...
			V3_Sub( &Delta, &Cursor, &OldPosition ) ;
			CActor *pActor = m_pMovie->GetCurActor() ;
			if( pActor )
			{
				pActor->KeyDrag( &Delta ) ;
			}
//			m_pDragKey->SetPosition( &Cursor ) ;
			Invalidate( ) ;
			GetDocument()->SetModifiedFlag( TRUE ) ;
...
void CMovieView::OnMovieExportGAMECUBE() 
{
	...
	if( m_pMovie )
	{
		...
		fd.m_ofn.lpstrTitle = "Export Movie For GameCube" ;
		fd.m_ofn.lpstrFilter = "Movie Files (*.mvi)\000*.mvi\000" ;

		if( fd.DoModal() == IDOK )
		{
			CString FileName = fd.GetPathName( ) ;
			GetDocument()->MovieExport( EXPORT_GAMECUBE, FileName, m_pMovie ) ;
		}
	}
}
...
void CMovieView::OnMovieExportPC() 
{
	...
	if( m_pMovie )
	{
		...
		fd.m_ofn.lpstrTitle = "Export Movie For PC" ;
		fd.m_ofn.lpstrFilter = "All Files (*.*)\000*.*\000" ;

		if( fd.DoModal() == IDOK )
		{
			CString FileName = fd.GetPathName( ) ;
			GetDocument()->MovieExport( EXPORT_PC, FileName, m_pMovie ) ;
		}
	}
}
...
void CMovieView::OnMovieExportXBOX() 
{
	...
	if( m_pMovie )
	{
		...
		fd.m_ofn.lpstrTitle = "Export Movie For XBox" ;
		fd.m_ofn.lpstrFilter = "Movie Files (*.mvi)\000*.mvi\000" ;

		if( fd.DoModal() == IDOK )
		{
			CString FileName = fd.GetPathName( ) ;
			GetDocument()->MovieExport( EXPORT_XBOX, FileName, m_pMovie ) ;
		}
	}
}
void CMovieView::OnMovieExportPS2() 
{
	...
	if( m_pMovie )
	{
		...
		fd.m_ofn.lpstrTitle = "Export Movie For PS2" ;
		fd.m_ofn.lpstrFilter = "Movie Files (*.mvi)\000*.mvi\000" ;

		if( fd.DoModal() == IDOK )
		{
			CString FileName = fd.GetPathName( ) ;
			GetDocument()->MovieExport( EXPORT_PS2, FileName, m_pMovie ) ;
		}
	}
}
