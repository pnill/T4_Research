...
void Simplify3DSMax( qraw* QRaw )
{
    ...
    for (i=0; i<QRaw->NMaterials; i++)
    {
        vector3d UVMin = { 1000, 1000,0};
        vector3d UVMax = {-1000,-1000,0};
...
err QRAW_CleanQRaw( qraw* QRaw )
{
    ...
    if( QRaw2 == NULL ) 
        RETURN_FAILURE("Not enough memory");
...
err QRAW_LoadQRaw( qraw* QRaw, char* FileName )
{
    ...
    TKN_Clear( &Text );
    if( TKN_ReadFile( &Text, FileName ) ) 
        RETURN_FAILURE(fs("Could not open file <%s>",FileName));
        ...
        while (Text.TokenType != TOKEN_EOF)
        {
            if ((Text.TokenType == TOKEN_DELIMITER) && (Text.Delimiter == '['))
            {
                ...
                if (1)
                {
                    ...
                    D0 = V3_Distance(&QT->XYZ[0],&QT->XYZ[1]);
                    D1 = V3_Distance(&QT->XYZ[1],&QT->XYZ[2]);
                    D2 = V3_Distance(&QT->XYZ[2],&QT->XYZ[0]);
...
err QRAW_GetMeshIndex( qraw* QRaw, s32* DestIndex, char* MeshName )
{
    ...
    if ( i == QRaw->NMeshes ) 
        RETURN_FAILURE(fs("Could not find mesh <%s>",MeshName));
...
err QRAW_DeleteMesh( qraw* QRaw, char* MeshName )
{
    ...
    //---------------------------------------------------------------------
    {
        ...
        T = x_realloc( QRaw->QTri, sizeof(qtri) * QRaw->NQTris );
        if ( T == NULL ) 
            RETURN_FAILURE("Not enough memory");
...
err QRAW_CopyMesh( qraw* DestQRaw, char* DestMeshName, qraw* SrcQRaw, char* SrcMeshName )
{
    ...
    //---------------------------------------------------------------------
    if ( QRAW_GetMeshIndex( DestQRaw, NULL, DestMeshNameUpper ) == ERR_SUCCESS )
        RETURN_FAILURE("Destination mesh already exists");

    //---------------------------------------------------------------------
    // Find meshes IDs
    //---------------------------------------------------------------------
    if ( QRAW_GetMeshIndex( SrcQRaw, &SID, SrcMeshName ) ) 
        RETURN_FAILURE("Source mesh cannot be found");
    ...
    if ( DestQRaw->QTri == 0 )
    {
        ...
        // Make sure that we don't waste memory
        T = x_malloc( sizeof(qtri) * NSTri );
        if ( T == NULL ) RETURN_FAILURE("Not enough memory");
    ...
    else
    {
        ...
        // Make sure that we don't waste memory
        T = x_realloc( DestQRaw->QTri, sizeof(qtri) * ( DestQRaw->NQTris + NSTri ) );
        if ( T == NULL ) RETURN_FAILURE("Not enough memory");
...
err QRAW_RenameMesh( qraw* QRaw, char* NameTo, char* NameFrom )
{
    ...
    if ( QRAW_GetMeshIndex( QRaw, &ID, NameFrom ) ) 
        RETURN_FAILURE(fs("Could not find mesh <%s> to rename",NameFrom));
...
err QRAW_MergeMesh( qraw* QRaw, char* MergeInto, char* MergeFrom )
{
    ...
    if ( QRAW_GetMeshIndex( QRaw, &DestID, MergeInto ) ) 
        RETURN_FAILURE(fs("Could not find mesh <%s> to merge into",MergeInto));

    if ( QRAW_GetMeshIndex( QRaw, &SrcID,  MergeFrom ) ) 
        RETURN_FAILURE(fs("Could not find mesh <%s> to merge from",MergeFrom));
...
void QRAW_Bounds( qraw* QRaw, vector3d** Min, vector3d** Max, f32* MaxDist )
{
    s32         i, j;
    vector3d    V;
    f32         L;
    ...
    *Min = (vector3d*)x_malloc( sizeof( vector3d ) * QRaw->NMeshes );
    *Max = (vector3d*)x_malloc( sizeof( vector3d ) * QRaw->NMeshes );
...
void QRAW_OrientedBounds( qraw* QRaw, vector3d** Bound )
{
    s32         i, j;
    vector3d*   BestMin;
    vector3d*   BestMax;
    ...
    *Bound      = (vector3d*)x_malloc( sizeof( vector3d ) * QRaw->NMeshes * 8);
    BestMin     = (vector3d*)x_malloc( sizeof( vector3d ) * QRaw->NMeshes);
    BestMax     = (vector3d*)x_malloc( sizeof( vector3d ) * QRaw->NMeshes);
    TempMin     = (vector3d*)x_malloc( sizeof( vector3d ) * QRaw->NMeshes);
    TempMax     = (vector3d*)x_malloc( sizeof( vector3d ) * QRaw->NMeshes);
    ...
    for (i=0; i<QRaw->NMeshes; i++)
    {
        vector3d* B  = &((*Bound)[i*8]);
        vector3d* MN = &BestMin[i];
        vector3d* MX = &BestMax[i];
        ...
        for (j=0; j<8; j++)
            V3_RotateY( &B[j], &B[j], -BestYAngle[i] );
    }
...
f32 QRAW_GetShortestEdge( qraw* QRaw )
{
    ...
    {
        vector3d V;
        V3_Sub( &V, &QRaw->QTri[0].XYZ[0], &QRaw->QTri[0].XYZ[1] );
        Shortest = V3_Length( &V );
    }
    
    // Loop through the facets.
    for( i = 0; i < QRaw->NQTris; ++i )
    {
        V3_Sub( &E1, &QRaw->QTri[i].XYZ[0], &QRaw->QTri[i].XYZ[1] );
        V3_Sub( &E2, &QRaw->QTri[i].XYZ[1], &QRaw->QTri[i].XYZ[2] );
        V3_Sub( &E3, &QRaw->QTri[i].XYZ[2], &QRaw->QTri[i].XYZ[0] );
                         ...
                         s32*  NTsRemoved )
{
    ...
    MergedNode   = x_malloc( QRaw->NQTris * 3 * sizeof(qraw_merge_node) );
    MergedNodeID = x_malloc( QRaw->NQTris * 3 * sizeof(s32) );
    ASSERT( MergedNode   );
    ASSERT( MergedNodeID );
...
void QRAW_ComputeNormals( qraw* QRaw )
{
    ...
    FacetNormal = (vector3d*)x_malloc( QRaw->NQTris * sizeof(vector3d) );
    ASSERT( FacetNormal );
    ...
    for( i = 0; i < QRaw->NQTris; ++i )
    {
        ...
        V3_Sub( &V1, &QRaw->QTri[i].XYZ[1], &QRaw->QTri[i].XYZ[0] );
        V3_Sub( &V2, &QRaw->QTri[i].XYZ[2], &QRaw->QTri[i].XYZ[0] );
        V3_Cross( &Normal, &V1, &V2 );
        V3_Normalize( &Normal );
    ...
    for( j = 0;   j < 3;            ++j )
    {
        ...
        {
            V3_Normalize( &QRaw->QTri[i].Normal[j] );
