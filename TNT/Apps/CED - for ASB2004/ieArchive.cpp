...
bool CieArchive::WriteTag(u16 Type, u16 Tag)
{
	...
	try
	{
		...
		m_wTagPosition = m_pFile->GetPosition() ;
...
bool CieArchive::PatchTagLength(u32 PatchPosition)
{
	...
	try
	{
		u32 OldPosition = m_pFile->GetPosition() ;
...
bool CieArchive::Writevector3d(vector3d Value)
{
	bool	Success = TRUE ;			// Success Code

	try
	{
		m_pFile->Write(&Value,sizeof(vector3d)) ;
	}
	catch(...)
	{
		Success = FALSE ;
...
bool CieArchive::WriteTaggedvector3d(u16 Tag, vector3d Value)
{
	bool	Success = TRUE ;			// Success Code

	Success &= WriteTag (IE_TYPE_vector3d, Tag) ;
	Success &= Writevector3d (Value) ;
...
bool CieArchive::Readvector3d(vector3d &Value)
{
	bool	Success = TRUE ;			// Success Code

	//---	Assert Type
	ASSERT (m_rType == IE_TYPE_vector3d) ;
	Success &= (m_rType == IE_TYPE_vector3d) ;

	try
	{
		m_pFile->Read(&Value,sizeof(vector3d)) ;
	}
	catch(...)
	{
		Success = FALSE ;
