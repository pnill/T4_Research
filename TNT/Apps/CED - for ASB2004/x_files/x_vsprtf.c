...
s32 dtoa( char* fmtbase, f64 fpnum, char cvt, s32 width, s32 prec )
{
    ...
    *ew       = 0;
    ...
    if (prec == 6) powprec = 1.0e6;
        ...
        powprec = vsprintf_pow(10.0, (long) prec);
...
reswitch:
        switch (ch)
        {
            ...
            case 'i':
                ...
                if (flags & QUADINT)
                {
                    uqval = x_va_arg(ap, s32); //************ N64 s32->s64

                    if ((s32)uqval < 0)      //************ N64 s32->s64
                    {
                        uqval = (u32)(-(s32)uqval);
                        sign = '-';
                    }
            ...
            case 'o':
                if (flags & QUADINT) uqval = x_va_arg(ap, s32); //****** N64 s32->s64
                else
                    ulval = GetUnSignARG( &ap, flags );
