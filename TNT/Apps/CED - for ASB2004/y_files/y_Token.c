...
err  TKN_ReadFile  ( tokenizer* Tokenizer, const char* Path )
{
    X_FILE* fh;
    ...
    x_fseek( fh, 0, SEEK_END );
    Tokenizer->FileSize = x_ftell(fh);
    x_fseek( fh, 0, SEEK_SET );
...
void TKN_DumpFile  ( tokenizer* Tokenizer )
{
    ...
    printf("%1c",Tokenizer->FileBuffer[i]);
}
...
void TKN_ShowFile  ( tokenizer* Tokenizer )
{
    ...
    while (Tokenizer->TokenType != TOKEN_EOF)
    {
        ...
        {
            case TOKEN_DELIMITER: printf("D:   %c \n",Tokenizer->Delimiter); break;
            case TOKEN_SYMBOL:    printf("S:   %s \n",Tokenizer->String); break;
            case TOKEN_STRING:    printf("Q:  '%s'\n",Tokenizer->String); break;
