...
public:
	...
	s32					m_Skin ;									// Skin to use with this actor
	...
	vector3 			*m_pCache ;									// Cache of Points
...
public:
	...
	void			SetSkin				( s32 Skin ) {m_Skin=Skin;}		// Set skin
	s32				GetSkin				( void ){return m_Skin;}		// Get skin
	...
	void			KeyDrag				( vector3  *pDelta ) ;			// Drag Selected Ones
	...
	void			CalcSpline			( vector3  *p1, vector3  *p2,
										  vector3  *v1, vector3  *v2,
										  int num, vector3  *points) ;	// Calc Spline Segment
	...
	void			GetRotationAtTime	( vector3  *pRot, int t ) ;		// Get Rotation at Time
	f32				GetScaleAtTime		( int t );						// Get scale at time
	void			PoseSkeleton		( int t ) ;						// Pose Skeleton at Time
	void			GetPositionAtTime	( vector3  *pv, int t ) ;		// Get Position at Time
	void			GetDirectionAtTime	( vector3  *pv, int t ) ;		// Get Direction at Time
