...
void CSkel::CopySkel( CSkel* SkelSrc )
{
    s32 i;
    ...
    if ( SkelSrc->m_NBones )
    {
        m_pBone = (SSkelBone*)x_malloc(sizeof(SSkelBone)*SkelSrc->m_NBones);
        ASSERT(m_pBone);
...
xbool CSkel::ImportASF( const char* FileName )
{
    s32			i, j;
    tokenizer	TOK;
    s32			NBonesRead;
    ...
    m_pBone = (SSkelBone*)x_malloc( sizeof(SSkelBone) * m_NBones );
    ASSERT( m_pBone );

    //---   clear bones to default values
    x_memset( m_pBone,0, sizeof(SSkelBone) * m_NBones );
    ...
    if ( m_NBones > 1 )
    {
        ...
        {
            SSkelBone* pBone = &m_pBone[NBonesRead];
...
s32 CSkel::FindBone( const char* BoneName )
{
    s32 i;
...
void CSkel::CalcGlobalPosFromLocal( void )
{
...
void CSkel::DecideMirrorBones( void )
{
    s32 i,j;
    char TestName[32];
    char* SrcName;
...
const char* CSkel::GetBoneName( s32 BoneID )
{
...
void CSkel::CalcLocalPosFromGlobal( void )
{
    SSkelBone* pParent;
    SSkelBone* pBone;
    s32         i;
...
void CSkel::DelBone( s32 TargetBoneID )
{
    s32         i,j;
    SSkelBone* pBone;
    ...
    pBone = (SSkelBone*)x_malloc( sizeof(SSkelBone) * (m_NBones - 1) );
    ASSERT(pBone);
