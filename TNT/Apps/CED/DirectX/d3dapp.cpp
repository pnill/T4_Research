...
//-----------------------------------------------------------------------------
BOOL CD3DApplication::OnSizeMove( LRESULT& rResult )
{
    if( m_bFrameMoving )
        DXUtil_Timer( TIMER_START );
    ...
    if( m_bActive && m_bWindowed )
    {
            ...
            m_rcWindowClient.bottom - m_rcWindowClient.top)
        {
            ...
            if( FAILED( rResult = Resize3DEnvironment() ) )
            {
...
HRESULT CD3DApplication::UserSelectNewDevice()
{
    ...
    if( IDOK != DialogBoxParam( (HINSTANCE)GetModuleHandle(NULL),
                                ...
                                (int (__stdcall*)(struct HWND__*,unsigned int, unsigned int, long))SelectDeviceProc, (LPARAM)this ) )
        return S_OK;
