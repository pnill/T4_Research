...
void CDlgCamera1::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LOCKTOFACEDIR, m_LockToFaceDir);
...
void CDlgCamera1::SetTarget( vector3  *pVector )
{
	m_EditsLocked = true ;
...
BEGIN_MESSAGE_MAP(CDlgCamera1, CDialog)
	...
	ON_BN_CLICKED(IDC_LOCKTOFACEDIR, OnLocktofacedir)
	//}}AFX_MSG_MAP
...
void CDlgCamera1::OnLocktofacedir() 
{
	// TODO: Add your control notification handler code here
	ASSERT( m_pView ) ;
	m_pView->SetLockCameraToFaceDir( m_LockToFaceDir.GetCheck() ? TRUE : FALSE ) ;
}
