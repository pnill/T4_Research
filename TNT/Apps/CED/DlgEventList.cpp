...
void CDlgEventList::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EVENT_FRAMESPIN, m_FrameSpin);
	...
	if( pMotion )
	{
		for( int i = 0 ; i < pMotion->GetNumEvents() ; i++ )
		{
			...
			if( pEvent->GetAttached() )
				Name.Format( "%03d - %s -> %s", pEvent->GetFrame(), pEvent->GetName(), pEvent->GetBoneName() ) ;
			else
...
//			V3_Sub( &vlocal, &v, &vframe );
		...
		//---	set the frame spinner range to the range of the animation
		m_FrameSpin.SetRange( 0, pMotion->GetNumFrames()-1 );
	}
...
//	m_ButtonAddCurrentEvent.EnableWindow( Enable );
	...
	GetDlgItem( IDC_BUTTON_ATTACH_BONENOW )->EnableWindow( Enable && !pEvent->GetAttached() );

	if( Enable )
	{
		if( pEvent->GetAttached() )
		{
			SetDlgItemText( IDC_BUTTON_DETACH, "Detach" );
			GetDlgItem( IDC_BUTTON_DETACH )->EnableWindow( Enable );
		}
		else
		{
			SetDlgItemText( IDC_BUTTON_DETACH, "Attach" );
			GetDlgItem( IDC_BUTTON_DETACH )->EnableWindow( Enable && (pEvent->GetBone()!=-1) );
		}
	}
	else
		GetDlgItem( IDC_BUTTON_DETACH )->EnableWindow( Enable );
...
void CDlgEventList::OnChangeEventFrame() 
{
	...
	if( !m_EditsLocked )
	{
		if( !m_List.GetSafeHwnd() || (m_List.GetCurSel() == -1) )
			return;
...
void CDlgEventList::OnButtonDetach()
{
	if( !m_pView )											return;
	if( !m_pView->m_pCharacter )							return;
	if( !m_pView->m_pCharacter->m_pCurMotion )				return;
	if( !m_pView->m_pCharacter->m_pCurMotion->m_pCurEvent )	return;

	//---	let the button toggle the attached status.
	CEvent* pEvent = m_pView->m_pCharacter->m_pCurMotion->m_pCurEvent;
	if( pEvent->GetAttached() )
		m_pView->m_pCharacter->m_pCurMotion->m_pCurEvent->SetAttached( FALSE );
	else
	{
		if( pEvent->GetBone() != -1 )
			m_pView->m_pCharacter->m_pCurMotion->m_pCurEvent->SetAttached( TRUE );
	}
