...
void CKey::Serialize(CieArchive& a)
{
	...
	if (a.IsStoring())
	{
		...
		a.WriteTaggedvector3 	( IE_TAG_KEY_POS,			m_Position ) ;
		a.WriteTaggedvector3 	( IE_TAG_KEY_VECTOR,		m_Vector ) ;
		a.WriteTaggedvector3 	( IE_TAG_KEY_ROTATION,		m_Rotation ) ;
	...
	else
	{
		...
		while (!done)
		{
			...
			case IE_TAG_KEY_VECTOR:
				a.Readvector3 ( m_Vector ) ;
				break ;
			case IE_TAG_KEY_ROTATION:
				a.Readvector3 ( m_Rotation ) ;
				break ;
...
void CKey::SetPosition( vector3  *pv )
{
	m_Position = *pv;
//	V3_Copy( &m_Position, pv ) ;
}

void CKey::GetPosition( vector3  *pv )
{
	*pv = m_Position;
//	V3_Copy( pv, &m_Position ) ;
}

void CKey::SetVector( vector3  *pv )
{
	m_Vector = *pv;
//	V3_Copy( &m_Vector, pv ) ;
}

void CKey::GetVector( vector3  *pv )
{
	*pv = m_Vector;
//	V3_Copy( pv, &m_Vector ) ;
}

void CKey::SetRotation( vector3  *pv )
{
	m_Rotation = *pv;
//	V3_Copy( &m_Rotation, pv ) ;
}

void CKey::GetRotation( vector3  *pv )
{
	*pv = m_Rotation;
