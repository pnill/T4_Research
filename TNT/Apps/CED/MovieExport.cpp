...
CMovieExport::ShortVector3::ShortVector3()
: X(0)
, Y(0)
, Z(0)
{
...
CMovieExport::ShortVector3::ShortVector3(const vector3 & v)
{
    X = static_cast<s16>( v.X+0.5f );
    Y = static_cast<s16>( v.Y+0.5f );
    Z = static_cast<s16>( v.Z+0.5f );
}

void CMovieExport::ShortVector3::EnsureNonzero()
{
    if ((X==0) && (Y==0) && (Z==0))
        Z = 1;
}

CExportFile& operator<<(CExportFile& out, const CMovieExport::ShortVector3& v)
{
    out.Writes16( v.X );
    out.Writes16( v.Y );
    out.Writes16( v.Z );
...
//	ACTORS
	...
	while( Pos )
	{
		...
		if( pActor )
		{
			...
			movie_ActorTable.Writes32( (s32)movie_Actors.GetPosition() ) ;		// Address of Keys
			{
...
//				s8 t = (s8)pDoc->m_CharacterList.PtrToIndex( pActor->m_pCharacter ) ;
			...
			while( PosKey )
			{
				...
				if( pKey )
				{
                    ...
                    ShortVector3 p16(p), v16(v), r16(r);
                    v16.EnsureNonzero();                    //Converts dir<0,0,0> to <0,0,1> so rotations on dir will work

                    movie_Actors << p16 << v16 << r16;            //Export key pos, dir, and rot info in s16 format
	...
	{
		//---	Output Actor Data to Table
		movie_ActorTable.Writes32( (s32)movie_Actors.GetPosition() ) ;		// Address of Shot list
		...
		while( Pos )
		{
			...
			if( pShot )
			{
				...
				t = (s16)pMovie->m_ActorList.PtrToIndex(pShot->GetTarget());		
											...
											movie_Actors.Writes16( 0 );
				//---	end of union
				t = 0x0002;														// Flags field - This character uses linear blend
				t |= pShot->GetInterpolate() ? 0x0080 : 0;
											movie_Actors.Writes16( t );
