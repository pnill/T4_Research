...
void Simplify3DSMax( qraw* QRaw )
{
    ...
    for (i=0; i<QRaw->NMaterials; i++)
    {
        vector3  UVMin( 1000, 1000,0);
        vector3  UVMax(-1000,-1000,0);
...
err QRAW_LoadQRaw( qraw* QRaw, char* FileName )
{
    ...
    TKN_Clear( &Text );
    if( TKN_ReadFile( &Text, FileName )!=ERR_SUCCESS ) 
        ...
        while (Text.TokenType != TOKEN_EOF)
        {
            if ((Text.TokenType == TOKEN_DELIMITER) && (Text.Delimiter == '['))
            {
                ...
                if (1)
                {
					...
					vector3 Diff;
					Diff = QT->XYZ[0] - QT->XYZ[1];
					D0 = Diff.Length();
					Diff = QT->XYZ[1] - QT->XYZ[2];
					D1 = Diff.Length();
					Diff = QT->XYZ[2] - QT->XYZ[0];
					D2 = Diff.Length();
...
void    QRAW_Transform     ( qraw* QRaw, matrix4* M )
{
    ...
    for (i=0; i<QRaw->NQTris; i++)
    {
        for (j=0; j<3; j++)
        {
			M->Transform( &V, &(QRaw->QTri[i].XYZ[j]), 1);
