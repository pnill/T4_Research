...
mesh::chunk& mesh::chunk::operator=( mesh::chunk& chunk )
{
	x_strcpy( m_Name, chunk.m_Name );

	m_nVerts = chunk.m_nVerts;
	m_pVertex = new mesh::vertex[m_nVerts];
	x_memcpy( m_pVertex, chunk.m_pVertex, m_nVerts*sizeof(mesh::vertex) );

	m_nFaces = chunk.m_nFaces;
	m_pFace = new mesh::face[m_nFaces];
	x_memcpy( m_pFace, chunk.m_pFace, m_nFaces*sizeof(mesh::face) );
