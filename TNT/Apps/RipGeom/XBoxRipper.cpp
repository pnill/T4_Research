...
//#include <windows.h>
//#include "d3d8-xbox.h"
// HACK! d3d8-xbox.h won't compile for some reason, so we'll just define this stuff ourselves
...
void ExportXBOXGeometry( mesh::object& rGeom, X_FILE* outfile )
{
    ...
    for( i = 0; i < rGeom.m_nChunks; i++ )
    {
        ...
        s_pMesh[i].VertUsage      = g_BuildTriStrips  ? 0               : 0;
        ...
        //  Build the submeshes
        {
            ...
            while ( NTotalTris < s_NTris )
            {
                ...
                if( g_BuildTriStrips )
                {
                    ...
                    u16*                pIndices    = (u16*)s_pMesh[i].pIndices;
					t_Triangle			*pTris;
                    s32                 nTris       = 0;
...
//                    BuildStrips( &s_TriData[NTotalTris], TriCount, &pStrips, NStrips, MAX_NUM_VERTS );
                    ...
                    ///////////////////////////////////////////////////////////////
                    s_pSubMesh[s_CurSubMesh].MeshID         = i;
                    ...
                    for( s = 0; s < NStrips; s++ )
                    {
                        ...
                        {
                            //  Add indices to mesh indices pool
                            ASSERT(s_pMesh[i].NIndices < MAX_NUM_PRIMS);
                            s32 dumb_fucking_hell = s_pMesh[i].NIndices;
                            pIndices[s_pMesh[i].NIndices] = pStrips[s].pVerts[vIndex];
                            
                            // Imcrement the number of indices in the mesh as well as the local counter.
                            s_pMesh[i].NIndices++;
                            NIndices++;
...
//        bImageIsALightmap = x_strstr( TestDir, "LIGHTMAPS" ) != NULL;
    ...
    {
        // Write out the textures
        for( i = 0; i < rGeom.m_nTextures; i++ )
        {
		    x_bitmap	BMP;
		    xbool		ret;

		    if( g_Verbose )
			    x_printf( "Loading %s\n", rGeom.m_pTexture[i].m_FileName );

		    if( !x_stricmp( rGeom.m_pTexture[i].m_FileName, "none" ) )
		    {
			    AUXBMP_SetupDefaultBitmap( BMP );
			    ret = TRUE;
		    }
		    else
		    {	ret = AUXBMP_LoadBitmap( BMP, rGeom.m_pTexture[i].m_FileName );
		    }
		    ...
		    else
		    {
			    ...
			    x_splitpath( rGeom.m_pTexture[i].m_FileName, Drive, Dir, Name, Ext );
			    ...
			    ret = AUXBMP_ConvertToXBOX( BMP, FALSE, FALSE, TRUE );
			    ...
			    {
				    if( g_UseMips && (BMP.GetWidth() > 8) && (BMP.GetHeight() > 8) )
				    {
					    BMP.BuildMips();
				    }
