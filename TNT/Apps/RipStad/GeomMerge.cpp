...
void QGeomMerge::AddGeom( mesh::object* pObject )
{
    ...
    // The latest Ascii Mesh is rotating all of our objects 90 counter clockwise, so we must do the reverse.
    radian3 StadiumRotateBias( 0.0f, R_90, 0.0f );
    m_pGeoms[m_NGeoms]->Rotate( StadiumRotateBias );
