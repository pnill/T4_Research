...
xbool   g_NightVersion = FALSE;
xbool   g_DynamicLighting = FALSE;
X_FILE* g_TextureNameFile = NULL;
X_FILE* g_CollisionFile = NULL;
X_FILE* g_GridPointFile = NULL;
X_FILE* g_TextOut = NULL;
X_FILE* g_NotFound = NULL;
...
{
    x_printf( "Usage: RipStad [options] <Input> <output.GDF>\n\n" );
    x_printf( "Options:\n" );
	...
	x_printf( "  -N .............. Build as a night stadium (use night textures)\n");
//    x_printf( "  -S .............. Build triangle strips\n" );  // not currently supported
    ...
    x_printf( "  -C <filename> ... Export collsion data to <filename>\n" );
    x_printf( "  -G <filename> ... Export grid point data to <filename>\n" );
...
//    g_TextOut = x_fopen( "DebugText.txt", "w" );
    ...
    while ( argc )
    {
        ...
        //---   check for building triangle strips  // NOT CURRENTLY SUPPORTED
        else
        if ( !x_strcmp( pArg, "-S" ) )
        {
        ...
        //---   should we export collision data?
        else
        if ( !x_strcmp( pArg, "-C" ) )
        {
            argc--;
            ...
            if ( !argc )
            {
                x_printf( "ERROR: Missing collision file name.\n" );
                OutputUsageMessage();
                PauseOnError();
                exit( 0 );
            }
            g_CollisionFile = x_fopen( argv[0], "wb" );
            if ( !g_CollisionFile )
            {
                x_printf( "ERROR: Unable to open %s for writing.\n", argv[0] );
                OutputUsageMessage();
        ...
        //---   should we export grid point data?
        else
        if ( !x_strcmp( pArg, "-G" ) )
        {
            argc--;
            ...
            if ( !argc )
            {
                x_printf( "ERROR: Missing grid point file name.\n" );
                OutputUsageMessage();
                PauseOnError();
                exit( 0 );
            }
            g_GridPointFile = x_fopen( argv[0], "wb" );
            if ( !g_GridPointFile )
            {
                x_printf( "ERROR: Unable to open %s for writing.\n", argv[0] );
                OutputUsageMessage();
    ...
    for ( i = 0; i < g_NInputFiles; i++ )
    {
        ...
        if ( !bStat )
        {
            ...
            g_NotFound = x_fopen( "NoLoad.TXT", "a+" );
            x_fprintf( g_NotFound, "Could not load %s\n", g_InputFile[i] );
            x_fclose( g_NotFound );
//            PauseOnError();
            exit( 0 );
        }
...
//    TempTf = new CTextFile();
 ...
 //   if ( g_Verbose ) x_printf( "INFILE: %s\n", g_InputFile[0] );

    //---   load the text file
//    bStat = TempTf->LoadTextFile( g_InputFile[0] );
//    if ( !bStat )
//    {
//        x_printf( "ERROR: Unable to load %s\n", g_InputFile[0] );
//        PauseOnError();
...
//    TheGeom.Load( *TempTf );
    ...
    if ( g_Verbose ) x_printf( "Removing Degenerate Polys... " );
    //---   remove the degenerate polys
    if (RipTarget != RIP_TARGET_PS2)
    ...
    {
        if ( g_Verbose ) x_printf( "Calculating Vertex Normals... " );
//        CalcVertNormals( &TheGeom );
    ...
    for ( i = 0; i < TheGeom.m_nChunks; i++ )
    {
        ...
        {
            if (RipTarget == RIP_TARGET_PS2 || RipTarget == RIP_TARGET_XBOX)
            {
                if ( g_BuildTriStrips == FALSE )
                {
    ...
    {
    case RIP_TARGET_PS2:
        if ( g_Verbose ) x_printf( "Exporting PS2 Geometry... " );
//        TheGeom.FixPS2CrowdUVs();
        MESHUTIL_RemoveDegenerateTris( &TheGeom );
        ExportPS2Geometry( TheGeom, fh );
        break;
    case RIP_TARGET_PC:
        if ( g_Verbose ) x_printf( "Exporting PC Geometry... " );
        ExportPCGeometry( TheGeom, fh );
        break;
    case RIP_TARGET_GAMECUBE:
        if ( g_Verbose ) x_printf( "Exporting GameCube Geometry... " );
        ExportGCGeometry( TheGeom, fh );
        break;
    case RIP_TARGET_XBOX:
        if ( g_Verbose ) x_printf( "Exporting XBOX Geometry... " );
