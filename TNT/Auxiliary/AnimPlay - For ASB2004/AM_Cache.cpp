...
//    s32         NStreams;
    ...
    NFrames = MIN( (AnimInfo->ExportNFrames - BlockID*(AnimGroup->FramesPerBlock-1)), AnimGroup->FramesPerBlock );
    ...
    // Count how many streams there are
//    NStreams = (AnimGroup->NBones*3)+3+2;
//    for( i = 0; i < 8; i++ )
//    {
//        if( AnimInfo->PropsPresent       & (1<<i)) NStreams += 6;
//        if( AnimInfo->PointsPresent      & (1<<i)) NStreams += 1;
//        if( AnimInfo->UserStreamsPresent & (1<<i)) NStreams += 1;
//    }
//    ASSERT( NStreams == AnimInfo->NStreams );
    ...
    while( 1 )
    {
        // Load raw data to memory and decompress
            ...
            if( ACCCMP_Decode( pCompData, (s16*)AMCACHE_DecompBuffer[DB].DataPtr, AnimInfo->NStreams, (s16)NFrames ) )
                break;
        #else
            if( ACCCMP_Decode( AMCACHE_CompDataPtr, (s16*)AMCACHE_DecompBuffer[DB].DataPtr, AnimInfo->NStreams, (s16)NFrames ) )
                break;
        #endif
...
{
    ASSERT( Anim );
    ASSERT( Anim->m_AnimInfo );

    // Confirm that no block is locked
    ASSERT( Anim->m_BlockData == NULL );

    // Load new block
    s32 FramesPerBlock = Anim->m_AnimGroup->FramesPerBlock-1;
    Anim->m_BlockID   = ((s32)Anim->m_CurrentExportFrame) / FramesPerBlock;
    Anim->m_BlockData = AMCACHE_LockAnimFrames( Anim->m_AnimGroup, Anim->m_AnimInfo->ID, Anim->m_BlockID );
    ASSERT( Anim->m_BlockData );

    // Compute block bounds
    Anim->m_BlockFrame0 = Anim->m_BlockID * FramesPerBlock;
    Anim->m_BlockFrameN = MIN( (Anim->m_BlockID+1) * FramesPerBlock, Anim->m_AnimInfo->ExportNFrames - 1 );
}
...
void AMCACHE_UnlockBlock( mocap_player* Anim )
{
    ...
    AMCACHE_UnlockAnimFrames( Anim->m_AnimGroup, Anim->m_AnimInfo->ID, Anim->m_BlockID );
    Anim->m_BlockData = NULL;
}
