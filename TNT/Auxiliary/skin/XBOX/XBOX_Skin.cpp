////////////////////////////////////////////////////////////////////////////
//
// XBOX_Skin.cpp - implementation of the skin file for the XBOX
//
////////////////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////////////////
// QSkin(XBOX) Vertex Shader info
////////////////////////////////////////////////////////////////////////////

#include "XBOX_VBShaderDefines.h"
#include "XBOX_VBShader.h"
#include "XBOX_VBShadowShader.h"
#include "XBOX_PShader.h"
#include "XBOX_PShader_Cheat.h"

////////////////////////////////////////////////////////////////////////////
// Definitions
////////////////////////////////////////////////////////////////////////////
//#define TRACE_SKIN_LOADS
...
s32 QSkin::s_PixelShaderID = -1;
s32 QSkin::s_PixelShader_CheatID = -1;

// Shadow buffer width and height.
#define SHADOWBUFFERWIDTH  512
#define SHADOWBUFFERHEIGHT 512
...
void QSkin::ActivateVertexShader( void )
{
    if( s_VertexShaderID == -1 )
        s_VertexShaderID = XBOX_RegisterVertexShader( s_SKIN_ObjShader, s_SKIN_VertexFormat, 0 );
...
void QSkin::ActivateShadowShader( void )
{
    if( s_ShadowShaderID == -1 )
        s_ShadowShaderID = XBOX_RegisterVertexShader( s_SKIN_ShadowShader, s_SKIN_VertexFormat, 0 );
...
void QSkin::ActivatePixelShader( void )
{
    if (XBOX_IsToonCheatActive())
    {
        if( s_PixelShader_CheatID == -1 )
            s_PixelShader_CheatID = XBOX_RegisterPixelShader( s_SKIN_PS_ShadowBuffer_Cheat );

        XBOX_ActivatePixelShader( s_PixelShader_CheatID );
        return;
    }
    if( s_PixelShaderID == -1 )
        s_PixelShaderID = XBOX_RegisterPixelShader( s_SKIN_PS_ShadowBuffer );

    XBOX_ActivatePixelShader( s_PixelShaderID );
}

//==========================================================================

void QSkin::DeActivatePixelShader( void )
{
    if (XBOX_IsToonCheatActive())
    {
        ASSERT( s_PixelShader_CheatID != -1 );

        XBOX_DeActivatePixelShader( s_PixelShader_CheatID );
        return;
    }
    ASSERT( s_PixelShaderID != -1 );

    XBOX_DeActivatePixelShader( s_PixelShaderID );
}
...
#if defined( TRACE_SKIN_LOADS )
	x_printf( "Code File:%s, Line:%d, Loading %s\n", __FILE__, __LINE__, filename );
#endif
...
void QSkin::SetupFromResource( X_FILE* fp )
{
    ...
    XBOXSkinD3DVert*    pBufferData;
    XBOXSkinFileHeader  SkinFileHeader;
    XBOXSkinMeshHeader  MeshHeader;
    s32                 MeshID;
    ...
    pD3DDevice = XBOX_GetDevice();

    // Read the header out of the file.
    VERIFY( x_fread( &SkinFileHeader, sizeof( XBOXSkinFileHeader ), 1, fp ) == 1 );
    ...
    // Allocate Resources
    ////////////////////////////////////////////////////////////////////////
    m_pMeshes       = new XBOXSkinMesh[m_NMeshes];
    m_pSubMeshes    = new XBOXSkinSubmesh[m_NSubMeshes];
    m_pBoneNames    = new char[m_NBones * SKIN_BONE_NAME_LENGTH];
    m_pTextureNames = new char[m_NTextures * X_MAX_PATH];

#if defined( TRACE_SKIN_LOADS )
	x_printf( "MeshName:%s, NMeshes:%d, NTextures:%d\n",m_Name, m_NMeshes, m_NTextures );
#endif
    ...
    {
        // Read Mesh Header
        VERIFY( x_fread(&MeshHeader, sizeof(XBOXSkinMeshHeader), 1, fp ) == 1 );
        ...
        m_pMeshes[MeshID].NMorphTargets  = 0;  // Not supported on XBOX yet.
        m_pMeshes[MeshID].pMorphTarget   = NULL;
        m_pMeshes[MeshID].NMorphDeltas   = 0;  // Not supported on XBOX yet.
    ...
    x_fread(m_pSubMeshes,    sizeof(XBOXSkinSubmesh), m_NSubMeshes, fp );
    x_fread(m_pTextureNames, sizeof(XBOXSkinTexture), m_NTextures, fp );
    x_fread(m_pBoneNames,    sizeof(XBOXSkinBone),    m_NBones, fp );
...
void QSkin::OpenSkinBuffer( s32 NumberOfSkins, s32 MaxSubMeshCountPerSkin )
{
    ...
    XBOXSkinBuffer::mMaxNumberOfSkins     = NumberOfSkins;
    XBOXSkinBuffer::mMaxNumberOfSubmeshes = NumberOfSkins * MaxSubMeshCountPerSkin;

    // Determine how much scratch memory is going to be required, then take it.
    MatrixDataSize        = sizeof( SXBOXMatrixGroup ) * NumberOfSkins;
    SubmeshDataSize       = sizeof( XBOXSubMeshGroup ) * XBOXSkinBuffer::mMaxNumberOfSubmeshes;
    XBOXSkinBuffer::mScratchDataSize = MatrixDataSize + SubmeshDataSize + 32;// add 32 for alignment purposes 

    XBOXSkinBuffer::mpSMEMData = SMEM_BufferAlloc( XBOXSkinBuffer::mScratchDataSize );
    ASSERT( XBOXSkinBuffer::mpSMEMData );

    // Allign the Scratch Memory pointer to a 32 byte value.
    XBOXSkinBuffer::mpSMEMData = (byte*)ALIGN_32( (u32)XBOXSkinBuffer::mpSMEMData );

    // Set the matrix pointer to the start of the Scratch memory space.
    XBOXSkinBuffer::mpMatrices = (SXBOXMatrixGroup*)XBOXSkinBuffer::mpSMEMData;

    // Set the SubmeshGroup pointer to wherever the end of the matricies are going to be.
    XBOXSkinBuffer::mpSubMeshGroup = (XBOXSubMeshGroup*)(XBOXSkinBuffer::mpSMEMData + MatrixDataSize);

    // Initialize the skin and submesh counts.
    XBOXSkinBuffer::mNSkinsInBuffer     = 0;
    XBOXSkinBuffer::mNSubmeshesInBuffer = 0;
}
...
//            continue;
        ...
        for( SubMeshID = m_pMeshes[MeshID].FirstSubMesh; SubMeshID < (m_pMeshes[MeshID].FirstSubMesh + m_pMeshes[MeshID].NSubMeshes); SubMeshID++ )
        {
            ...
            {
                // For whatever reason it is assumed that the last testure in the array of textures used
                // for a submesh is the environment map texture.  Who knew?  MJS - May 29, 2003
            ...
            XBOXSkinBuffer::AddSubmeshGroup( &SubMeshGroup );
        }
    }

    // Mark the addition of the skin to the buffer.
    XBOXSkinBuffer::UpdateSkinCount( );
}
...
void QSkin::FlushSkinBuffer( void )
{
    ...
    if (XBOX_IsToonCheatActive())
        XBOX_ActivateToonTexture();

    s32                 SubMeshID, BoneID;
    XBOXSkinMesh*       pActiveMesh     = NULL;
    ...
    XBOXSubMeshGroup*   pCurrentSubMeshGroup;

    LPDIRECT3DDEVICE8   pD3DDevice;

    // If there are no submeshes in the buffer, just leave now.
    if( XBOXSkinBuffer::mNSubmeshesInBuffer == 0 )
        return;
    ...
    SMEM_StackPushMarker();
    pSubMeshOrder = (SSortSubMesh*)SMEM_StackAlloc( sizeof(SSortSubMesh) * XBOXSkinBuffer::mNSubmeshesInBuffer );
    ASSERT( pSubMeshOrder != NULL );

    //--- init sort structures
    for( SubMeshID = 0; SubMeshID < XBOXSkinBuffer::mNSubmeshesInBuffer; ++SubMeshID )
    {
        pCurrentSubMeshGroup = &XBOXSkinBuffer::mpSubMeshGroup[SubMeshID];
    ...
    RadixSort( XBOXSkinBuffer::mNSubmeshesInBuffer, pSubMeshOrder );
    ...
    ActivatePixelShader();
    static FLOAT fAmbient[4] = { 0.68f, 0.680f, 0.700f, 1.0f };
    pD3DDevice->SetPixelShaderConstant( 0, fAmbient, 1 );
    // Go through each sub mesh in the skin that has been sorted and draw it.
    for( SubMeshID = 0; SubMeshID < XBOXSkinBuffer::mNSubmeshesInBuffer; SubMeshID++ )
    {
        pCurrentSubMeshGroup = &XBOXSkinBuffer::mpSubMeshGroup[ pSubMeshOrder[SubMeshID].Index ];
...
//                ENG_SetBlendMode( ENG_BLEND_FIXED_ALPHA | ENG_BLEND_ADDITIVE, 50 );
        ...
        {
            VRAM_XBOX_Activate( *pCurrentSubMeshGroup->pBitmap2, 1 );
        ...
        if( pCurrentSubMeshGroup->pMatrices->BoneMatrices != pActiveMatrices )
        {
            ...
            {
                DXWARN( pD3DDevice->SetVertexShaderConstant( (3*BoneID), &pActiveMatrices[BoneID].M, 3 ));
            }
        }
        //---------------------------------------
        ...
        if( pCurrentSubMeshGroup->Flags & SKIN_SUBMESH_FLAG_SHADOW )
        {
            ENG_SetRenderFlags( ENG_ZBUFFER_TEST_ON | ENG_ZBUFFER_FILL_ON | ENG_ALPHA_TEST_OFF | ENG_ALPHA_BLEND_ON );
            u8  ShadowAlpha = 30;
            //ENG_SetBlendMode( ENG_BLEND_NORMAL | ENG_BLEND_FIXED_ALPHA, ShadowAlpha );
            ...
            for( ShadowPass = 0; ShadowPass < pCurrentSubMeshGroup->NumShadows; ShadowPass++ )
            {
                ...
                XBOX_SetStencilMode( ENG_STENCIL_ON, pCurrentSubMeshGroup->pAlphaLevels[ShadowPass] );
                // set the ZBias so each overlap shadow will not fight
                XBOX_SetShadowBiasMode( ENG_SHADOW_BIAS_ON, 16 - ShadowPass );
...
//                if( pCurrentSubMeshGroup->pMesh != pActiveMesh )
                {
                    ...
                    DXWARN( pD3DDevice->SetStreamSource( 0, pActiveMesh->pVertexBuffer, sizeof(XBOXSkinD3DVert) ));
            ...
            XBOX_SetShadowBiasMode( ENG_SHADOW_BIAS_OFF );
        }
        else
        {
...
*/
    ...
    radian3         Rot;                // XYZ rotation values.
    vector3         Trans;              // POSition translation value.              
    const vector3*  pCurrentBoneScale;
    vector3         OriginToBone;    
    
    for( BoneID = 0; BoneID < pAnimGroup->NBones; BoneID++ )
    {
        pCurrentMatrix = const_cast<matrix4*>(&pBoneMatrices[BoneID]);
...
#endif
        // Transpose the matrix and copy it into the animgroup matrices.
        D3DXMatrixTranspose( (D3DXMATRIX *)&pMatrixGroup->BoneMatrices[BoneID].M, (D3DXMATRIX *)&pCurrentMatrix->M );
    }
}
...
//-------------------------------------------------------------------------------------------------------------------------------------
void QSkin::DrawSkeleton( SXBOXMatrixGroup* pMatrixGroup, xbool bMirror, void* pAnimGroupPointer, f32 JointScale )
{
    s32         BoneID;
    s32         FirstJoint;
    s32         SecondJoint;
    s32         NBones;
    vector3     pBonesPos[SKIN_MAX_MATRICES];
    vector3     BoneLine[2];
    color       BoneColors[2];
    matrix4     TransMatrix;
    anim_group* pAnimGroup;

    DRAW_SetMode( DRAW_3D | DRAW_FILL | DRAW_NO_TEXTURE | DRAW_ALPHA | DRAW_NO_ZBUFFER | DRAW_CLIP | DRAW_LIGHT );

    NBones = GetNBones( );
    BoneColors[0].Set( 255, 255, 255, 255 );
    BoneColors[1] = BoneColors[0];

    pAnimGroup = (anim_group*)pAnimGroupPointer;

    // Transform the vertex.
    for( BoneID = 0; BoneID < NBones; BoneID++ )
    {
        // Transform the point.
        if( bMirror )
        {
            pBonesPos[BoneID] = pAnimGroup->Bone[pAnimGroup->Bone[BoneID].MirrorID].OriginToBone;
            pBonesPos[BoneID].X = -pBonesPos[BoneID].X;
        }
        else
        {
            pBonesPos[BoneID] = pAnimGroup->Bone[BoneID].OriginToBone;
        }

        D3DXMatrixTranspose( (D3DXMATRIX *)&TransMatrix, (D3DXMATRIX *)&pMatrixGroup->BoneMatrices[BoneID].M );

        pBonesPos[BoneID] = TransMatrix.Transform( pBonesPos[BoneID] );

        // Draw the actual bone location in 3D space.
        DrawJoint( pBonesPos[BoneID], JointScale, &pMatrixGroup->BoneMatrices[BoneID] );
    }

    // Draw the bones themselves.
    for( BoneID = 0; BoneID < NBones; BoneID++ )
    {
        if( bMirror )
        {
            FirstJoint  = pAnimGroup->Bone[pAnimGroup->Bone[BoneID].MirrorID].ParentID;
            SecondJoint = pAnimGroup->Bone[pAnimGroup->Bone[BoneID].MirrorID].BoneID;
        }
        else
        {
            FirstJoint  = pAnimGroup->Bone[BoneID].ParentID;
            SecondJoint = pAnimGroup->Bone[BoneID].BoneID;
        }

        if( FirstJoint != -1 && SecondJoint != -1 )
        {
            BoneLine[0] = pBonesPos[FirstJoint];
            BoneLine[1] = pBonesPos[SecondJoint];
            DRAW_Lines( 2, BoneLine, BoneColors );
        }
    }
}
