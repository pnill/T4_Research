...
typedef struct GHIConnection
{
	...
	GHIBuffer recvBuffer;         // The buffer for incoming data.
	...
	int fileBytesReceived;        // Number of file bytes received.
	int totalSize;                // Total size of the file, -1 if unknown.
	...
	unsigned long lastThrottleRecv;  // The last time we received on a throttled connection.
