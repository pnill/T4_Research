...
)
{
	...
	iconnection->retryConnect = GPIFalse;
	...
	do
	{
		...
		while((next = strstr(iconnection->socketBuffer.buffer, "\\final\\")) != NULL)
		{
			...
			if(len > iconnection->inputBufferSize)
			{
				...
				tempPtr = (char*)gsirealloc(iconnection->inputBuffer, iconnection->inputBufferSize + 1);
				if(tempPtr == NULL)
					Error(connection, GP_MEMORY_ERROR, "Out of memory.");
				iconnection->inputBuffer = tempPtr;
			}
			memcpy(iconnection->inputBuffer, iconnection->socketBuffer.buffer, len + 1);
			...
			memmove(iconnection->socketBuffer.buffer, next, iconnection->socketBuffer.len + 1);
...
*/
	...
	if(iconnection->connectState == GPI_CONNECTING)
	{
		do
		{
			...
			//PANTS|07.10.00 - only sleep if looping
			loop = (GPIBool)((result == GP_NO_ERROR) && (blockingOperationID != 0) && (iconnection->connectState == GPI_CONNECTING));
			if(loop)
				msleep(10);
		}
		while(loop);
	...
	// Retry connect?
	/////////////////
	if(iconnection->retryConnect && gpiFindOperationByID(connection, &operation, 1))
	{
		// Copy off the nick/email/password.
		////////////////////////////////////
		char * nick = goastrdup(iconnection->nick);
		char * email = goastrdup(iconnection->email);
		char * password = goastrdup(iconnection->password);

		// Connect.
		///////////
		result = gpConnect(connection, nick, email, password, iconnection->firewall?GP_FIREWALL:GP_NO_FIREWALL, operation->blocking?GP_BLOCKING:GP_NON_BLOCKING, operation->callback.callback, operation->callback.param);

		// freeclear the copies.
		///////////////////
		freeclear(nick);
		freeclear(email);
		freeclear(password)
...
)
{
	...
	if(profile->buddyStatus)
	{
		...
		if(profile->buddyStatus->statusString)
			temp += (strlen(profile->buddyStatus->statusString) + 1);
		if(profile->buddyStatus->locationString)
			temp += (strlen(profile->buddyStatus->locationString) + 1);
		nBuddyMemory += temp;
		nTotalMemory += temp;
	}
	if(profile->cache)
	{
		...
		if(profile->cache->nick)
			temp += (strlen(profile->cache->nick) + 1);
		if(profile->cache->email)
			temp += (strlen(profile->cache->email) + 1);
		if(profile->cache->firstname)
			temp += (strlen(profile->cache->firstname) + 1);
		if(profile->cache->lastname)
			temp += (strlen(profile->cache->lastname) + 1);
		if(profile->cache->homepage)
			temp += (strlen(profile->cache->homepage) + 1);
		nInfoMemory += temp;
		nTotalMemory += temp;
	}
