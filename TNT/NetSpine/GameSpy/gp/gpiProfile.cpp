...
)
{
	...
	return ((unsigned int)profile->profileId % numBuckets);
}
...
)
{
	...
	return (GPIBool)!TableMapSafe2(iconnection->profileList.profileTable, gpiProfileMapCallback, &mapData);
}
...
{
	return (GPIBool)(profile && !profile->cache && !profile->buddyStatus && !profile->peerSig && !profile->authSig);
}
