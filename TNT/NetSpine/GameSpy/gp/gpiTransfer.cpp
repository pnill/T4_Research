...
#define GPI_DATA_SIZE             (8 * 1024)
//#define GPI_CONFIRM_FILES

#ifdef __MWERKS__ // CodeWarrior requires prototype
GPResult gpiSendTransferReply(GPConnection * connection, GPITransferID * transferID, GPIPeer * peer, int result, const char * msg);
#endif
...
)
{
	...
	CHECK_RESULT(gpiAddCallback(connection, iconnection->callbacks[GPI_TRANSFER_CALLBACK], arg, NULL, GPI_ADD_TRANSFER_CALLBACK));
...
)
{
	...
	else
	{
		...
		// Check the md5.
		/////////////////
		md5Failed = (memcmp(localMD5, remoteMD5, 32) != 0);
...
)
{
	...
	gpiDebug(connection, "HNDLDATA(PT): %d\n", bufferLen);

	// Write the data.
	//////////////////
	writeFailed = (fwrite(buffer, 1, bufferLen, file->file) != (size_t)bufferLen);
	if(writeFailed)
	{
		// Flag the errors.
		///////////////////
...
)
{
	...
	// Add the file index.
	//////////////////////
	gpiSendOrBufferStringLen(connection, transfer->peer, "\\file\\", 6);
...
)
{
	...
	else
	{
		...
		else
		{
			...
			if(total)
				gpiDebug(connection, "SENTTOTL(PT): %d\n", total);
...
)
{
	...
	// Start the message.
	/////////////////////
	CHECK_RESULT(gpiPeerStartTransferMessage(connection, peer, GPI_BM_FILE_SEND_REPLY, (GPITransferID_st)transferID));
