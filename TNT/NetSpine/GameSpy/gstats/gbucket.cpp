...
void *BucketConcat(bucketset_t set, char *name, void *value)
{
	...
	s = (char*)DoGet(pbucket);
	temp = (char *)gsimalloc(strlen(s) + strlen((char*)value) + 1);
	strcpy(temp,s);
	strcat(temp, (char*)value);
...
static void DumpMap(void *elem, void *clientData)
{
	...
	if (data->maxlen <= minlen) 
	{
		if (data->maxlen == 0)
			...
			data->maxlen *= 2;
		data->data = (char*)gsirealloc(data->data, data->maxlen);
	...
	{	
	case bt_int:
		data->len += sprintf(data->data + data->len,"\\%s\\%d",bucket->name,bucket->vals.ival);
		break;
	case bt_float:
		data->len += sprintf(data->data + data->len,"\\%s\\%f",bucket->name,bucket->vals.fval);
		break;
	case bt_string:
		data->len += sprintf(data->data + data->len,"\\%s\\%s",bucket->name,bucket->vals.sval);
		break;
...
static int BucketHash(const void *elem, int numbuckets)
{
    ...
    len = strlen(s);
    for (i = 0; i < len ; i++)
	  hashcode = hashcode * MULTIPLIER + tolower(s[i]);
