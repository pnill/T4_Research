...
int InitStatsConnection(int gameport)
{
	...
	if (get_sockaddrin(StatsServerHostname,SSPORT,&saddr,NULL) == 0)
		return GE_NODNS;
...
int SendGameSnapShot(statsgame_t game, const char *snapshot, int final)
{
	...
	data = (char *)gsimalloc(snaplen + 256);
	...
	while (SocketReadable(sock))
	{
		if (rcvmax - rcvlen < 128) //make sure there are at least 128 bytes gsifree in the buffer
		{
			if (rcvmax == 0)
				...
				rcvmax *= 2;
			rcvbuffer = (char*)gsirealloc(rcvbuffer, rcvmax+1);
			if (rcvbuffer == NULL)
				return 0; //errcon
		}
...
static void SetPersistDataHelper(int localid, int profileid, persisttype_t type, int index, char *data, int len, PersDataSaveCallbackFn callback, void *instance, int kvset)
{
	...
	{
		senddata = (char *)gsimalloc(len + tlen + 256);
...
static void CallReqCallback(int reqindex, int success, char *data, int length)
{
	...
	if (req->callback)
		...
		case rt_datacb:
			...
			break;
		case rt_savecb:
			((PersDataSaveCallbackFn )req->callback)(req->localid, req->profileid, req->pdtype, req->pdindex,success, req->instance);
			break;
		case rt_profilecb:
...
{
	int *ret;
	DoFunc(func, game, name, &value, bt_int, (void*)ret);
	return *(int *)ret;
...
{
	double *ret;
	DoFunc(func, game, name, &value, bt_float, (void*)ret);
	return *(double *)ret;
...
{
	char *ret;
	DoFunc(func, game, name, value, bt_string, (void*)ret);
	return ret;
