...
static unsigned short GetLocalPort(SOCKET sock)
{
	...
	ret = getsockname(sock,(struct sockaddr *)&saddr, (unsigned int*)&saddrlen);
	if (ret == SOCKET_ERROR)
		return 0;
...
static void SendInitPackets(NATNegotiator neg)
{
	...
	if (p->usegameport && !neg->initAckRecv[NN_PT_GP])
	{
		...
		SendPacket(neg->gameSock, matchup1ip, MATCHUP_PORT, p, INITPACKET_SIZE);
	...
	if (!neg->initAckRecv[NN_PT_NN1])
	{
		...
		SendPacket(neg->negotiateSock, matchup1ip, MATCHUP_PORT, p, INITPACKET_SIZE);
	}	
	...
	if (!neg->initAckRecv[NN_PT_NN2])
	{
		...
		SendPacket(neg->negotiateSock, matchup2ip, MATCHUP_PORT,p, INITPACKET_SIZE);
	}
...
{
	NATNegotiator neg = AddNegotiator();
	if (!ResolveServers())
		return ne_dnserror;
	if (neg == NULL)
	...
	if (neg->negotiateSock != INVALID_SOCKET)
	{
		...
		while (1)
		{
			if(!CanReceiveOnSocket(neg->negotiateSock))
				break;
			//else we have data
			error = recvfrom(neg->negotiateSock, indata, NNINBUF_LEN, 0, (struct sockaddr *)&saddr, (unsigned int*)&saddrlen);
...
void NNProcessData(char *data, int len, struct sockaddr_in *fromaddr)
{
	...
	if (!CheckMagic((unsigned char*)data))
