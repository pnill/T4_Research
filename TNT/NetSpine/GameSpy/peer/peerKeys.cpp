...
)
{
	...
	if(num)
	{
		...
		chatGetGlobalKeys(connection->chat, nick?nick:ROOM, num, (const char **)data.keys, piGetGlobalKeysCallback, peer, CHATFalse);
		gsifree(data.keys);
	}
	...
	if(!nick)
	{
		...
		if(num)
		{
			...
			assert(data.next == num);
			chatGetChannelKeys(connection->chat, ROOM, nick?nick:"*", num, (const char **)data.keys, piGetRoomKeysCallback, peer, CHATFalse);
			gsifree(data.keys);
		}
	}
}
