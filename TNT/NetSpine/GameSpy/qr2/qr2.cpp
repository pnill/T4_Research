...
qr2_error_t qr2_create_socket(/*[out]*/SOCKET *sock, const char *ip, /*[in/out]*/int * port)
{
	...
	if (baseport == 0) //we bound it dynamically
	{
		...
		lasterror = getsockname(hbsock,(struct sockaddr *)&saddr, (unsigned int*)&saddrlen);
		if (lasterror)
			return e_qrbinderror;
...
void qr2_check_queries(qr2_t qrec)
{
	...
	while (1)
	{
		if(!CanReceiveOnSocket(qrec->hbsock))
			return;
		//else we have data
		error = recvfrom(qrec->hbsock, indata, INBUF_LEN - 1, 0, (struct sockaddr *)&saddr, (unsigned int*)&saddrlen);
