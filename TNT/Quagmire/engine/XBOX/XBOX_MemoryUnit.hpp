#ifndef XBOX_MEMORY_UNIT_HPP
#define XBOX_MEMORY_UNIT_HPP
...
#define XBOXMU_MAX_PATH_LENGTH                      (260) // Found in MAX_PATH in the Windef.h file.
#define XBOX_HARD_DISK_NAME                         "Xbox hard disk"

/*
    QUAG_MEMORY_UNIT_ID_0 --> XBOX_USER_HARD_DISK_UNIT
*/
...
//
// Do to lack of time, XBOXMemoryUnitManager is now only going to allow you to save to the Hard Disk.
//=====================================================================================================================================
class XBOXMemoryUnitManager
{
    public:
        //-------------------------------------------------------------------------------------------------------------------------------------
        // Construction / Initialization
        //-------------------------------------------------------------------------------------------------------------------------------------
        XBOXMemoryUnitManager( void );
        ...
        //-------------------------------------------------------------------------------------------------------------------------------------
        void    Update( void );
        xbool   WasAMemoryCardConnectedLastFrame( void ) { return mbXBOXMemoryCardInsertionDetectedLastFrame; }
...
protected:
        ...
        void    SetAndFillCurrentXBOXSaveGameData( s32 SaveGameType, s32 SaveGameIndex, XGAME_FIND_DATA* pXboxFindGameData );
        XGAME_FIND_DATA* GetCurrentXBOXSaveGameData( s32 SaveGameType, s32 SaveGameIndex );
        ...
        xbool   MatchUpXBoxSaveToExpectedSaveGameType( XGAME_FIND_DATA* pXBoxSave ); 
...
protected:
        xbool   mInitialized;
        xbool   mbXBOXMemoryCardInsertionDetectedLastFrame;

        //-------------------------------------------------------------------------------------------------------------------------------------
        // Command Related Data.
        //-------------------------------------------------------------------------------------------------------------------------------------
        void    (XBOXMemoryUnitManager::*mpCurrentCommandHandler)(void);
        s32     mCurrentCommand;
        s32     mCommandState;
        ...
        XBOXMUResourceSaveGameType* mpSaveGameTypes;        // Info on each one of those types.

        SaveGameTypeAvailableList*  mpAvailableSavedGames;  // A running list of which saved games are available on the active card.
        XGAME_FIND_DATA*            mpXBOXSaveGameData;     // XBox specific save game info.
        XGAME_FIND_DATA*            mpCurrentXBOXSaveGameData;
        s32                         mMaxXBoxSavesForThisApp;    // The maximum number of valid saves that this app is expecting.
        ...
        WCHAR                       mWideNameBuffer[MEMORY_UNIT_SAVE_GAME_TITLE_LENGTH];
        XBOXFileInfo                mFileInfo;
        ...
        //-------------------------------------------------------------------------------------------------------------------------------------
        XBOXMemoryUnit              mHardDisk;
        xbool                       mbHardDiskIsSelected;
        char                        mActiveBaseDirectory[XBOXMU_MAX_PATH_LENGTH];
};

//=====================================================================================================================================
// Globals
//=====================================================================================================================================
extern XBOXMemoryUnitManager* gpXBOXMemoryUnitManager;

#endif // XBOX_MEMORY_UNIT_HPP
