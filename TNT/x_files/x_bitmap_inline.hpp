...
inline s32 x_bitmap::GetDataSize( s32 MipID ) const
{
    ...
    if (m_NMips)
    {
        miptable *table = (miptable*)m_pData;
        if (m_Flags & FLAG_DATA_PS2COMPRESSED)
            return (table[ MipID + 1 ].Offset - table[ MipID ].Offset );    // assuming table has dummy tail
        else
            return (table[ MipID ].W * table[ MipID ].H * s_Format[ m_Format ].BPP) / 8;
    }
    else
        return m_DataSize;
} 
...
{
    m_Flags &= ~(FLAG_COMPRESSED_S3TC | FLAG_DATA_PS2COMPRESSED);
}

inline xbool x_bitmap::IsCompressed( void ) const
{
#if defined( TARGET_PS2 ) || defined( TARGET_PC )
    // test for the flag and fall-thru if not set.
    if (m_Flags & FLAG_DATA_PS2COMPRESSED)
        return (m_Flags & FLAG_DATA_PS2COMPRESSED);
#endif
    return (m_Flags & FLAG_COMPRESSED_S3TC);
}

inline xbool x_bitmap::IsPS2Compressed( void ) const
{
#if defined( TARGET_PS2 ) || defined( TARGET_PC )
    // test for the flag and fall-thru if not set.
    if (m_Flags & FLAG_DATA_PS2COMPRESSED)
...
inline xbool x_bitmap::IsDataSystem( void ) const
{
    return (m_Flags & FLAG_DATA_INSYSTEMMEM);
}
