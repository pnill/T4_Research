...
//
// *set<>::x_reinsert(iterator, key)  - this method is an efficient way to
...
//
// *Key - This type must support the == operator, and if it is a class or struct
//    it needs to provide a functional copy constructor.
//
////////////////////////////////////////////////////////////////////////////
//
// TEMPLATE PARAMETERS
...
//                must be supported before using the set comparison operators.
...
//        operator=     - Copies contents of another set into this set.
//        operator==    - Determines if the contents of two sets are the same.
//                      
//    set::iterator     
...
//  
//    N is number of elements inside of the set.
//
//    set
//        size          O(1)
//        begin         O(1)
...
// class Foobar { ... };
//
// typedef x_std::set< Foobar* > FoobarSet;
// 
// FoobarSet  myFoobarSet;
//
// myFoobarSet.insert( 1 );
// myFoobarSet.insert( 2 );
//
// FoobarSet::iterator it;
// 
// it = myFoobarSet.find( 1 );
//
// X_STD_ASSERT( it != myFoobarSet.end() );
// X_STD_ASSERT( *it == 1 );
//
// it = myFoobarSet.begin();
// 
// X_STD_ASSERT( *it == 1 );
//
// ++it;
...
namespace x_std
{
    ...
    class set
    {
        ...
        private:
            ...
            typedef set<Key, Predicate, Allocator>                      set_type;
            typedef x_std_private::x_tree<Key, value_type, x_Kfn, Predicate, Allocator>  tree_type;
            typedef tree_type::iterator_pair                            iterator_pair;
            typedef tree_type::const_iterator_pair                      const_iterator_pair;
        ...
        public:
            ...
            typedef tree_type::const_reverse_iterator                   reverse_iterator;
            ...
            /////////////////////
            // Construction
            explicit set(const Predicate& comp = Predicate(), const Allocator& al = Allocator());
            set(const set<Key, Predicate, Allocator>&);
            set(const value_type *first, const value_type *last, const Predicate& comp = Predicate(), const Allocator & al = Allocator());
            ...
            inline iterator                 begin()                             { return const_cast<const set*>(this)->m_tree.begin();          }
            inline iterator                 end()                               { return const_cast<const set*>(this)->m_tree.end();            }
            inline reverse_iterator         rbegin()                            { return const_cast<const set*>(this)->m_tree.rbegin();         }
            inline reverse_iterator         rend()                              { return const_cast<const set*>(this)->m_tree.rend();           }
            inline iterator                 find(const Key& k_)                 { return const_cast<const set*>(this)->m_tree.find(k_);         }
            inline iterator                 upper_bound(const Key& k_)          { return const_cast<const set*>(this)->m_tree.upper_bound(k_);  }
            inline iterator                 lower_bound(const Key& k_)          { return const_cast<const set*>(this)->m_tree.lower_bound(k_);  }
            inline pair<iterator, iterator> equal_range(const Key& k_)          { return const_cast<const set*>(this)->m_tree.equal_range(k_);  }
            ...
            inline x_bool                    operator==(const set& rhs) const    { return m_tree == rhs.m_tree;   }
            inline x_bool                    operator<(const set& rhs) const     { return m_tree <  rhs.m_tree;   }
            ...
            inline void                     insert(const value_type* first_,
                   ...
                   void                     x_reinsert(iterator& it_, const Key& k_);
            ...
            inline void                     swap(set& s_)                            {        m_tree.swap(s_.m_tree);        }
    ...
    template <class x_K, class x_P, class x_A>
    set<x_K, x_P, x_A>& set<x_K, x_P, x_A>::operator=(const set<x_K, x_P, x_A>& rt)
    {
        m_tree = rt.m_tree;
    ...
    template <class x_K, class x_P, class x_A>
    set<x_K, x_P, x_A>::set(const x_P &p, const x_A &a)
    : m_tree(p, a)
    {}
    
    template <class x_K, class x_P, class x_A>
    set<x_K, x_P, x_A>::set(const set<x_K, x_P, x_A>& copy)
    : m_tree()
    {
    ...
    template <class x_K, class x_P, class x_A>
    set<x_K, x_P, x_A>::set(const value_type *first, const value_type *last, const x_P& comp, const x_A& al)
    : m_tree(comp, al)
    {
        insert(first, last);
    }
    ...
    template <class x_K, class x_P, class x_A>
    void set<x_K, x_P, x_A>::x_reinsert(iterator& it_, const x_K &k_)
    {
        tree_type::iterator it( it_ );

        m_tree.reinsert(it, k_);

        it_ = it;
    }

    template <class x_K, class x_P, class x_A> 
    x_bool operator != ( const set<x_K, x_P, x_A> & lhs , const set<x_K, x_P, x_A> & rhs )
    {
        return ! ( lhs == rhs ) ; 
    }
